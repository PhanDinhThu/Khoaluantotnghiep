...
public class AccountServlet_Add_Del extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if(acs == "" ) {
        	access_error = "Vui lòng chọn quyền truy cập !";
		...
		// Định dạng mail
           ...
           System.out.println("Thêm thành công tài khoản" + username );
		// Validate Username
		if (username.equals("")) {
		...
		if (!matcher2.matches()) {
			username_error = "Tên tài khoản không gồm kí tự đặc biệt!";
        } 
		...
		if(!phone.equals("") && phone.length()>12 || phone.length()<8){
			phone_error = "Số điện thoại phải >=9 & <= 11 ký tự số !";
			}
		if (!matcher1.matches()) {

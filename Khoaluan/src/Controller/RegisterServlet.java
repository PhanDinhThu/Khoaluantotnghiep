...
public class RegisterServlet extends HttpServlet {
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		String emailPattern = "^[\\w!#$%&ï¿½*+/=?`{|}~^-]+(?:\\.[\\w!#$%&ï¿½*+/=?`{|}~^-]+)*@(?:[a-zA-Z0-9-]+\\.)+[a-zA-Z]{2,6}$";
	      ...
	      //định dạng tentaikhoan
	          Pattern pattern1 = Pattern.compile("^[a-zA-Z0-9-]*$");
	          Matcher matcher2 = pattern1.matcher(username);
		// Validate Tai Khoan
		if (username.equals("")) {
			username_error = "Tên tài khoản không được để trống!";
		} else {
			if (taiKhoanDAO.checkAccount(username) == true) {
				username_error = "Tên tài khoản này đã tồn tài, vui lòng nhập tên khác !";
			}
		}
		if (!matcher2.matches()) {
			username_error = "Tên tài khoản không được kí tự đặc biệt!";
        } 
		...
		// Validate Mat Khau
		if (password1.equals("")) {
			password1_error = "Mật khẩu không được để trống !";
		}
		if (password2.equals("")) {
			password2_error = "Xác nhập lại mật khẩu không được để trống !";
		}
		...
		if (password1.toUpperCase().length() != password2.toUpperCase().length()) {
			password2_error = "Mật khẩu không khớp nhau !";
		}
		if (password2_error.length() > 0) {
			request.setAttribute("password2_error", password2_error);
		}
		...
		// Validate Email
		if (email.equals("")) {
			email_error = "Địa chỉ Email không được để trống!";
		} else {
			if (taiKhoanDAO.checkEmail(email) == true) {
				email_error = "Email này đã tồn tài, vui lòng nhập Email khác !";
			}
		}
		...
		// Validate Ho Va Ten
		if (fullname.equals("")) {
			fullname_error = "Họ và tên không được để trống !";
		}
		...
		if(!phone.equals("") && phone.length()>12 || phone.length()<8){
			phone_error = "Số điện thoại phải >= 9 và <= 11 ký tự số!";
			}
		if (!matcher1.matches()) {
		...
		// Validate Dia Chi
		if (address.equals("")) {
			address_error = "Địa chỉ không được để trống!";
		}
		...
		try {
			if (username_error.length() == 0 && password_error.length() == 0 && email_error.length() == 0
				...
				url = "/register.jsp";
				SendMail.sendMail(acc.getEmail(), "Home",

...
public class InvoiceDAOImpl implements InvoiceDAO {
	...
	public void insertInvoice(Invoice hd) {
		...
		try {
			...
			ps.setString(8, hd.getID_Employee());
			ps.setString(9, hd.getTime_Limit());
			ps.setString(10, hd.getNote());
			ps.executeUpdate();
			ketNoi.close();
	...
	public void updateInvoice(Invoice hd) {
		Connection ketNoi = DBConnection.getConnection();
		String sql = "UPDATE Invoice SET Status_Order=?, ID_Employee=? WHERE ID_Invoice=? ";
		try {
			PreparedStatement ps = ketNoi.prepareStatement(sql);
			
			ps.setInt(1, hd.getStatus_Order());
			ps.setString(2, hd.getID_Employee());
			ps.setString(3, hd.getID_Invoice());
			ps.executeUpdate();
			ketNoi.close();
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public void updateInvoice1(Invoice hd) {
		Connection ketNoi = DBConnection.getConnection();
		String sql = "UPDATE Invoice SET Status_Order=?, Time_Limit=? WHERE ID_Invoice=? ";
		try {
			PreparedStatement ps = ketNoi.prepareStatement(sql);
			
			ps.setInt(1, hd.getStatus_Order());
			ps.setString(2, hd.getTime_Limit());
			ps.setString(3, hd.getID_Invoice());
			ps.executeUpdate();
			ketNoi.close();
		} catch (SQLException e) {

...
//				list.add(bill);
	...
	public ArrayList<Export> getListExport() {
		Connection ketNoi = DBConnection.getConnection();
		String sql = "SELECT  InvoiceDetail.ID_Product product_id,Product.ID_Category category_id ,Product.Name_Product product_name,Product.Image image ,SUM(InvoiceDetail.Quantity) quantity\r\n" + 
				...
				"									GROUP BY InvoiceDetail.ID_Product, Product.Name_Product, Product.ID_Category, Product.Image, Product.Quantity";
		ArrayList<Export> list = new ArrayList<>();
		try {
			PreparedStatement ps = ketNoi.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				Export export = new Export( rs.getString("product_id"),rs.getString("category_id"), rs.getString("product_name"), rs.getString("image"),rs.getInt("quantity"));
				list.add(export);
			}
			ketNoi.close();
		} catch (SQLException e) {

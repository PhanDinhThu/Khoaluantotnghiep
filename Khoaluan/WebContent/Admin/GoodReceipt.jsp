					...
					<div class="col-md-12">
						...
						<div class="btn btn-success" style="padding: 0px 0px;" >
							...
							<!-- <button data-button="addReceipt" id="btnAddReceipt" class="btn btn-success" style="color: white;text-transform:uppercase;font-size:14px;">Thêm Phiếu Nhập</button> -->
								 <a class="navbar-brand" href="/Khoaluan/Admin/goodreceipt_insert.jsp" style="color: white;text-transform:uppercase;font-size:14px;"> Thêm Phiếu Nhập</a> 
						</div>
					</div>
				</div>
				<hr />
				...
				<div class="row">
					<div class="col-md-12">
						<div class="panel panel-default">
							<div class="panel-heading" style="background:#106356; color:black; font-weight:bold">QUẢN LÝ PHIẾU NHẬP  <span style="color:white;float:right"><span style="color:black; text-transform:uppercase">Tổng vốn nhập:</span>  <%= nf.format(count)%> VNĐ</span></div>
					...
					<%
						...
						<td style="text-align:center">
							 <a <% if(gr.getTotal() > 0 || !user.getID_Account().equals(a)){ %> style="display:none"<%}else{ %> style=" display: block;padding:3px 0px;margin-left:30%;width:35%"
									...
									%> 
							href="/Khoaluan/ReceiptServlet_Delete?action=Delete&ID_Receipt=<%=gr.getID_Receipt()%>" onclick="return confirm('Bạn có chắc chắn xóa mã phiếu nhập là <%=gr.getID_Receipt()%> không ?')"

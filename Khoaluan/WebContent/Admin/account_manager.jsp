					...
					<div class="col-md-12">
						...
						<div class="btn btn-success" style="padding: 0px 0px;" >
							<button type="button" class="navbar-toggle"
								...
								<a class="navbar-brand" href="/Khoaluan/Admin/account_insert.jsp" style="color: white;text-transform:uppercase;font-size:14px;"> Thêm Thành Viên</a>
						</div>
					</div>
				</div>
				...
				<div class="row">
					<div class="col-md-12">
						...
						<div class="panel panel-default">
							<div class="panel-heading" style="background:#106356; color:black; font-weight:bold">QUẢN LÝ THÀNH VIÊN</div>
								...
								<div class="table-responsive">
									<table class="table table-striped table-bordered table-hover"
										...
										<tbody>
											...
											%>
											<tr  class="odd gradeX">
												<td <% if ((user.getAccess() == 2 && acc.getAccess() ==1) ||( user.getAccess() == 2 && acc.getAccess() ==3)) {											
						...
						%>style="display:none" <%}%>class="center" style="text-align: center;">
												...
												<%
													if (acc.getAccess() == 3) {
												%>
												<a class="btn btn-warning btn-xs" >Nhân viên</a>
												<%
													}
												%>
						...
						%>style="display:none" <%}%> class="center" style="text-align: center;">
													...
													if (user.getAccess() ==2 && acc.getAccess() == 3    ) {											
														%>style="display:none" <%}
													else if (acc.getAccess() == 1 ) {											
												        %>style="display:none" <%}
													else if ((user.getAccess() == 2  && acc.getAccess() == 3)) {											
												        %>style="display:none" <%}
                                                    else{ %> style=" display:block"
													<%
													}
												%>  href="/Khoaluan/Admin/account_edit.jsp?action=Update&ID_Account=<%=acc.getID_Account()%>&Password=<%=acc.getPassword()%>&Username=<%=acc.getUsername()%>&Email=<%=acc.getEmail() %>&Fullname=<%=acc.getFull_Name() %>&Address=<%=acc.getAddress() %>&Phone=<%=acc.getSDT() %>"
													class="btn btn-info btn-xs" style="margin-bottom:5px">Sửa</a> 
													<a <% String ID_Account = acc.getID_Account();
													if (acc.getAccess() == 1 || user.getAccess() == 3 ) {											
														%>style="display:none" <%}
												...
												%>  href="/Khoaluan/AccountServlet_Delete?action=Delete&ID_Account=<%=acc.getID_Account()%>" onclick="return confirm('Bạn chắc chắn muốn xóa tài khoản <%=acc.getUsername()%> chứ ?')"
